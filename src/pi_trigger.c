...
unsigned long pi_trigger_command (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	trigger_rule_t *r;
	
	for (r = ruleListCommand.next; r != &ruleListCommand; r = r->next) {
		if (strcmp (argv[0], r->arg)) continue;
...
unsigned long pi_trigger_login (plugin_user_t *user, void *dummy, unsigned long event, buffer_t *token) {
	trigger_rule_t *r;
	
	for (r = ruleListLogin.next; r != &ruleListLogin; r = r->next) {
		if ( (r->cap & user->rights) != r->cap) continue;
		trigger_verify (r->trigger);
...
trigger_rule_t *trigger_rule_create (trigger_t *t, unsigned long type, unsigned long cap, unsigned char *arg, unsigned char *help) {
	trigger_rule_t *rule;
	trigger_rule_t *list;
...
int trigger_save (unsigned char *file) {
	...
	for (rule = ruleListCommand.next; rule != &ruleListCommand; rule = rule->next) {
		fprintf (fp, "rule %s %lu %lu %s %s\n", rule->trigger->name, rule->type, rule->cap, rule->arg, rule->help ? (char *)rule->help : "");
	}
...
unsigned long pi_trigger_handler_triggeradd (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	unsigned int type;
	trigger_t *t;
...
unsigned long pi_trigger_handler_triggerdel (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	trigger_t *t;
	...
	switch (trigger->type) {
		case TRIGGER_TYPE_FILE:
			return bf_printf (buf, "Trigger %s dumps file %s (Hits %ld)\n", 
				trigger->name, 
				trigger->file, trigger->usecnt);
			break;
		case TRIGGER_TYPE_COMMAND:
			return bf_printf (buf, "Trigger %s dumps text %*s (Hits %ld)\n", 
				trigger->name, 
				bf_used(trigger->text), trigger->text->s, trigger->usecnt);
			break;
	};
	return 0;
}

unsigned long pi_trigger_handler_triggerlist (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	trigger_t *t;
	unsigned int count;
...
unsigned long pi_trigger_handler_ruleadd (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	unsigned int type;
	trigger_t *t;
	trigger_rule_t *r;
...
unsigned long pi_trigger_handler_ruledel (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	trigger_t *t;
	trigger_rule_t *r;
	unsigned long id;
...
unsigned long pi_trigger_handler_rulelist (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	trigger_t *t;
	trigger_rule_t *r;
...
unsigned long pi_trigger_event_save (plugin_user_t *user, buffer_t *output, void *dummy, unsigned long event, buffer_t *token) {
        trigger_save (pi_trigger_SaveFile);
        return PLUGIN_RETVAL_CONTINUE;
}

unsigned long pi_trigger_event_load (plugin_user_t *user, buffer_t *output, void *dummy, unsigned long event, buffer_t *token) {
...
int pi_trigger_init () {
	...
	plugin_request (plugin_trigger, PLUGIN_EVENT_SAVE, (plugin_event_handler_t *)&pi_trigger_event_save);
        plugin_request (plugin_trigger, PLUGIN_EVENT_LOAD, (plugin_event_handler_t *)&pi_trigger_event_load);
