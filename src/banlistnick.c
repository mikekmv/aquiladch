...
banlist_nick_entry_t * banlist_nick_add (banlist_nick_t* list, unsigned char *nick, buffer_t *reason, unsigned long expire) {
...
unsigned int banlist_nick_del_bynick (banlist_nick_t* list, unsigned char *nick) {
	...
	dllist_foreach (l, e)
		if (!strncasecmp (e->nick, nick, NICKLENGTH)) break;
...
unsigned int banlist_nick_save (banlist_nick_t* list, unsigned char *file) {
	...
	dlhashlist_foreach (list, i) {
		...
		for (e = (banlist_nick_entry_t *)lst->dllist.next; e != lst; e = (banlist_nick_entry_t *)e->dllist.next) {
			if (e->expire && (now.tv_sec > e->expire))
				continue;
