...
banlist_client_entry_t * banlist_client_add (banlist_client_t* list, unsigned char *client, double minVersion, double maxVersion, buffer_t *reason) {
...
unsigned int banlist_client_del_byclient (banlist_client_t* list, unsigned char *client) {
	...
	dllist_foreach (l, e)
		if (!strncmp (e->client, client, NICKLENGTH)) break;
...
banlist_client_entry_t * banlist_client_find (banlist_client_t* list, unsigned char *client, double version) {
	banlist_client_entry_t *e, *l;
	...
	if (version == 0) return e;
	
	if ((version >= e->minVersion)&&(version <= e->maxVersion)) return 0;
