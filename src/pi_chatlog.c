...
unsigned long pi_chatlog_handler_chatlog (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
...
unsigned long pi_chatlog_handler_chat (plugin_user_t *user, void *priv, unsigned long event, buffer_t *token) {
	unsigned char *c;
	
	if (!chatlogmax) return PLUGIN_RETVAL_CONTINUE;
...
int pi_chatlog_init () {
	...
	config_register ("chatlog.lines", CFG_ELEM_ULONG, &chatlogmax,  "Maximum number of chat history lines.");
