...
float cpu_calculate (int time) {
	...
	old = &(procstats[level][(current[level] - time + PROCSTAT_MEASUREMENTS)%PROCSTAT_MEASUREMENTS]);
...
unsigned int bandwidth_printf (buffer_t *buf) {
	...
	bf_printf (buf, "Total traffic since boot: In %lu, Out %lu\n", hubstats.TotalBytesReceived, hubstats.TotalBytesSend);

	lvl  = &stats.bandwidth[0];
	elem = &lvl->probes [lvl->current ? lvl->current-1 : STATS_NUM_MEASUREMENTS];

	out = (8 * (double)elem->BytesSend)     / ( (double)elem->stamp.tv_sec*1000 + (double)elem->stamp.tv_usec/1000);
	in  = (8 * (double)elem->BytesReceived) / ( (double)elem->stamp.tv_sec*1000 + (double)elem->stamp.tv_usec/1000);
	...
	for (i = 0; i < STATS_NUM_LEVELS; i++) {
		...
		if (timerisset (&lvl->TotalTime)) {
			out = (8000 * (double)lvl->TotalBytesSend)     / ( (double)lvl->TotalTime.tv_sec*1000 + (double)lvl->TotalTime.tv_usec/1000);
			in  = (8000 * (double)lvl->TotalBytesReceived) / ( (double)lvl->TotalTime.tv_sec*1000 + (double)lvl->TotalTime.tv_usec/1000);
		
			retval += bf_printf (buf, "Average over %d seconds: In %f kbps, Out %f kbps\n", lvl->TotalTime.tv_sec, in/1000, out/1000);
		};
	};
	return retval;
}
...
/****************************************************************************************/
unsigned long pi_statistics_event_cacheflush (plugin_user_t *user, void *dummy, unsigned long event, buffer_t *token) {
	cpu_measure();
...
unsigned long pi_statistics_handler_statcache (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	buffer_t *buf;
...
unsigned long pi_statistics_handler_statbw (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	buffer_t *buf;
...
unsigned long pi_statistics_handler_statbuffer (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	buffer_t *buf;
...
unsigned long pi_statistics_handler_statcpu (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	buffer_t *buf;
	struct timeval now;
...
unsigned long pi_statistics_handler_statmem (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	buffer_t *buf;
	struct timeval now;
...
unsigned long pi_statistics_handler_uptime (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	struct timeval now;
...
#include "nmdc_protocol.h"
unsigned long pi_statistics_handler_statnmdc (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
...
int pi_statistics_init () {
	...
	command_register ("statnmdc",  &pi_statistics_handler_statnmdc, 0, "Show nmdc protocol stats. Experts only.");
