...
unsigned int plugin_user_next (plugin_user_t **user) {	
	...
	while (u && (u->state != PROTO_STATE_ONLINE)) u = u->next;
...
unsigned int plugin_user_findnickban (buffer_t *buf, unsigned char *nick) {
	...
	gettimeofday (&now, NULL);
	if (ne->expire) {
		return bf_printf (buf, "Found nick ban for %s for %lus because: %.*s", ne->nick, ne->expire - now.tv_sec, bf_used (ne->message), ne->message->s);
	} else {
		return bf_printf (buf, "Found permanent nick ban for %s because: %.*s", ne->nick, bf_used (ne->message), ne->message->s);
	}
}
...
unsigned int plugin_user_findipban (buffer_t *buf, unsigned long ip) {
	...
	ipa.s_addr = ip;
	if (ie->expire) {
		return bf_printf (buf, "Found IP ban for %s for %lus because: %.*s", inet_ntoa (ipa), ie->expire - now.tv_sec, bf_used (ie->message), ie->message->s);
	} else {
		return bf_printf (buf, "Found permanent ban for %s because: %.*s", inet_ntoa (ipa), bf_used (ie->message), ie->message->s);
	}
}
...
unsigned int plugin_user_forcemove (plugin_user_t *user, unsigned char *destination, buffer_t *message) {
	user_t *u;
...
unsigned int plugin_user_priv (plugin_user_t *src, plugin_user_t *target, plugin_user_t *user, buffer_t *message, int direct) {
	user_t *u, *t, *s;
	...
	ASSERT (u->state == PROTO_STATE_VIRTUAL);
	return direct ? ((plugin_private_t *)target->private)->proto->chat_priv_direct (u, t, s, message) : ((plugin_private_t *)target->private)->proto->chat_priv (u, t, s, message);
}
...
plugin_user_t    *plugin_robot_add (unsigned char *name, unsigned char *description, plugin_event_handler_t *handler) {
	user_t *u;
...
plugin_event_handler_t * plugin_robot_set_handler (plugin_user_t *robot, plugin_event_handler_t *handler) {
	user_t *u;
	plugin_private_t *priv;
...
int plugin_request (plugin_t *plugin, unsigned long event, plugin_event_handler_t *handler) {
	...
	if (event > PLUGIN_EVENT_NUMBER) return -1;
...
int plugin_ignore (plugin_t *plugin, unsigned long event) {
	...
	for (request = manager->eventhandles[event].next; request != &manager->eventhandles[event]; request =request->next)
		if (request->plugin == plugin) break;
...
int plugin_init () {
	...
	manager->num = PLUGIN_MAX_PLUGINS;
        ...
        /*
        config_register ("ConfigFile",    CFG_ELEM_STRING, &ConfigFile,  "Save file that stores all the hub settings.");
        config_register ("HardBanFile",   CFG_ELEM_STRING, &HardBanFile, "Save file that stores the hard bans.");
        config_register ("SoftBanFile",   CFG_ELEM_STRING, &SoftBanFile, "Save file that stores the softbans.");
        config_register ("NickBanFile",   CFG_ELEM_STRING, &NickBanFile, "Save file that stores the nickbans.");
        config_register ("AccountsFile",  CFG_ELEM_STRING, &AccountsFile, "Save file that stores the accounts.");
        */
