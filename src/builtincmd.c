...
/* say command */
unsigned long handler_say (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	return 0;
}

/* say command */
unsigned long handler_shutdown (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	exit(0);
	return 0;
}

/* say command */
unsigned long handler_version (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	bf_printf (output, "This is " HUBSOFT_NAME " Version " AQUILA_VERSION);
	return 0;
}

unsigned long handler_massall (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i;
	buffer_t *buf;
...
unsigned long handler_report (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i;
	buffer_t *buf;
	plugin_user_t *tgt = NULL;
	...
	*buf->e = '\0';
	...
	if (!tgt) {
		bf_printf (output, "Report could not be sent, %s not found.", **reporttarget->val.v_string ? *reporttarget->val.v_string : (unsigned char *)"target");
		return 0;
	}
...
unsigned long handler_myip (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	struct in_addr ip;
...
unsigned long handler_kick (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i = 0;
	buffer_t *buf;
	...
	*buf->e = '\0';
	...
	bf_printf (output, "Kicked user %s (ip %s) because: %.*s", target->nick, inet_ntoa (ip), bf_used(buf), buf->s);
	...
	if ((c = strcasestr (buf->s, "_BAN_"))) {
		...
		if (KickNoBanMayBan || (user->rights & CAP_BAN)) {
			...
			if ( (KickMaxBanTime == 0) || ((KickMaxBanTime > 0)&&(total <= KickMaxBanTime)) || (user->rights & CAP_BAN)) {
				if (total != 0) {
					bf_printf (output, "\nBanning user for ");
...
unsigned long handler_zombie (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
...
unsigned long handler_unban (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	if (argc < 2) {
		bf_printf (output, "Usage: %s <nick>", argv[0]);
		return 0;
	}
...
unsigned long handler_unbanip (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	struct in_addr  ip;
...
unsigned long handler_unbanip_hard (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	struct in_addr  ip;
...
unsigned long handler_banip (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i = 0;
	...
	*buf->e = '\0';
	...
	for (i = period ? 3 : 2; i < argc; i++)
        ...
        if (target) {
        	ip.s_addr = target->ipaddress;
        	if (!period) {
			bf_printf (output, "IP Banning user %s (ip %s) forever: %.*s", target->nick, inet_ntoa (ip), bf_used (buf), buf->s);
		} else {
...
unsigned long handler_bannick (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i = 0;
	unsigned long period = 0;
	buffer_t *buf;
	plugin_user_t *target;
...
unsigned long handler_ban (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i = 0;
	unsigned long period = 0;
	buffer_t *buf;
	plugin_user_t *target;
	...
	*buf->e = '\0';
	...
	if (!period) {
		bf_printf (output, "Banning user %s (ip %s) forever: %.*s", target->nick, inet_ntoa (ip), bf_used (buf), buf->s);
	} else {
...
unsigned long handler_banhard (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i = 0;
	unsigned long period = 0;
	buffer_t *buf;
	plugin_user_t *target;
	...
	*buf->e = '\0';
	for (i = period ? 3 : 2; i < argc; i++)
		...
		ip.s_addr = target->ipaddress;
		if (!period) {
			bf_printf (output, "HARD Banning user %s (ip %s) forever: %.*s", target->nick, inet_ntoa (ip), bf_used (buf), buf->s);
		} else {
		...
		if (inet_aton (argv[1], &ip)) {
			if (!period) {
				bf_printf (output, "HARD Banning ip %s forever: %.*s", inet_ntoa (ip), bf_used (buf), buf->s);
			} else {
...
unsigned long handler_banlist (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	struct in_addr ip;
...
unsigned long handler_help (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int j, hash;
	command_t *cmd, *list;
	...
	for (j = 1; j < argc; j++) {
		...
		for (cmd = list->next; cmd != list; cmd = cmd->next)
			if (!strcmp (cmd->name, argv[j])) break;
...
unsigned long handler_groupadd (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	...
	if (cap & ~user->rights) {
		...
		command_flags_print ((command_flag_t *)(Capabilities+CAP_PRINT_OFFSET), output, cap & ~user->rights);
		bf_strcat (output, "\n");
		goto leave;
	}
...
unsigned long handler_groupcap (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned long cap = 0, ncap = 0;
	account_type_t *type;
	...
	if (ncap & ~user->rights) {
		...
		command_flags_print ((command_flag_t *)(Capabilities+CAP_PRINT_OFFSET), output, ncap & ~user->rights);
		bf_strcat (output, "\n");
		goto leave;
	}
...
unsigned long handler_groupdel (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	account_type_t *type;
...
unsigned long handler_grouplist (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	account_type_t *type;
...
unsigned long handler_userlist (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	account_type_t *type;
	account_t *account;
	unsigned int i, cnt;
...
unsigned long handler_useradd (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned long cap = 0, ncap = 0;
	account_type_t *type;
	account_t *account;
	...
	command_flags_print ((command_flag_t *)(Capabilities+CAP_PRINT_OFFSET), output, account->rights | type->rights);
	bf_strcat (output, "\n");
	if (plugin_user_find (argv[1]))
		bf_printf (output, "User is already logged in. Tell him to rejoin to gain all rights of his new account.\n");

leave:
	return 0;
}

unsigned long handler_usercap (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned long cap = 0, ncap = 0;
	account_t *account;
	...
	if (account->classp->rights & ncap) {
		...
		command_flags_print ((command_flag_t *)(Capabilities+CAP_PRINT_OFFSET), output, ncap & account->classp->rights);
		bf_printf (output, " from his group!\n");
	}

	
	bf_printf (output, "User %s created with group %s.\nCurrent rights:", account->nick, account->classp->name);
	command_flags_print ((command_flag_t *)(Capabilities+CAP_PRINT_OFFSET), output, account->rights | account->classp->rights);
	bf_strcat (output, "\n");
	if (plugin_user_find (argv[1]))
		bf_printf (output, "User is already logged in. Tell him to rejoin to gain all his new rights.\n");

leave:
	return 0;
}

unsigned long handler_userdel (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	account_t *account;
	...
	if (plugin_user_find (argv[1]))
		bf_printf (output, "User is still logged in with all rights of the deleted account. Kick the user to take them away.\n");

leave:
	return 0;
}

unsigned long handler_userinfo  (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	...
	if (target) {
		...
		bf_printf (output, "Hubs: (%u, %u, %u), Slots %u\n", target->hubs[0], target->hubs[1], target->hubs[2], target->slots);
	};

	return 0;
}

unsigned long handler_usergroup  (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	account_t *account = NULL;
...
unsigned long handler_passwd  (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	account_t *account = NULL;
	unsigned char *passwd;
	
	if (argc < 2) {
		bf_printf (output, "Usage: %s [<nick>] <password>\n If no nick is specified, your own password is changed.", argv[0]);
		return 0;
	}
	...
	if ((strlen (passwd) < MinPwdLength) || !strcmp (passwd, account->passwd)) {
		bf_printf (output, "The password is unacceptable, please choose another. Minimum length is %d\n", MinPwdLength);
		goto leave;
	}
...
int printconfig (buffer_t *buf, config_element_t *elem) {
	switch (elem->type) {
		...
		command_flags_print ((command_flag_t *)(Capabilities+CAP_PRINT_OFFSET), buf, *elem->val.v_cap);
		bf_strcat (buf, "\n");
		break;
	    case CFG_ELEM_INT:
		...
		bf_printf (buf, "%s \"%s\"\n", elem->name, *elem->val.v_string ? *elem->val.v_string : (unsigned char *)"(NULL)");
		break;
	    default:
...
unsigned long handler_configshow  (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i;
...
unsigned long handler_confighelp  (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned int i;
	config_element_t *elem;
...
unsigned long handler_configset  (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
	unsigned long cap = 0, ncap = 0;
	config_element_t *elem;
...
unsigned long handler_save  (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
...
unsigned long handler_load  (plugin_user_t *user, buffer_t *output, void *priv, unsigned int argc, unsigned char **argv) {
