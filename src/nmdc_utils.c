...
buffer_t *rebuild_myinfo (user_t *u, buffer_t *b) {
	...
	/* normal users */
	l = 12 + NICKLENGTH + 1 + config.MaxDescriptionLength + config.MaxTagLength + 2 + 2 + config.MaxSpeedLength + 1 + config.MaxEMailLength + 1 + config.MaxShareLength+ 2;
	d = bf_alloc (l);
	...
	while ((*e != '$')&&(e < b->e)) e++;
	l = e - s;
	if (!l) goto nuke; /* speed entries aren't allowed to be 0. they should at least contain a single character.*/ 
...
buffer_t *zline (buffer_t *input) {
	...
	bf_strcat (output, "$Z ");
	for (w = work->s, o = output->e, e = output->buffer + output->size; (w < work->e)&&(o < e); w++, o++) {
		switch (*w) {
			case '\\':
				*o++ = '\\';
				*o   = '\\';
