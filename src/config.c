...
config_element_t *config_register (unsigned char *name, config_type_t type, void *ptr, const unsigned char *help) {
	int i;
	unsigned long hash;
	...
	for (elem = config_sorted.onext; elem != &config_sorted; elem = elem->onext) {
	        switch (elem->type) {
	            ...
	            case CFG_ELEM_STRING:
	                fprintf (fp, "%s \"%s\"\n", elem->name, *elem->val.v_string ? *elem->val.v_string : (unsigned char *)"");
	                break;                                                          
	            default:
