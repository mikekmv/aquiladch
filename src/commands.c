...
unsigned int command_flags_parse (command_flag_t *flags, buffer_t *buf, unsigned int argc, unsigned char **argv, unsigned int flagstart, unsigned long *flag, unsigned long *nflag) {
        unsigned int i, j;
        unsigned char *name;
...
int command_register (unsigned char *name, command_handler_t *handler, unsigned long cap, unsigned char *help) {
...
unsigned long cmd_parser (plugin_user_t *user, plugin_user_t *target, void *priv, unsigned long event, buffer_t *token) {
	unsigned int hash, i;
	buffer_t *local;
	unsigned char *c, t;
	unsigned int argc = 0;
	...
	c++; /* skip < */
	while (*c && (*c!='>')) c++;
	if (!*c) return PLUGIN_RETVAL_DROP;
	c++; /*  skip '>' */
	c++; /* skip ' ' */
...
unsigned long cmd_parser_priv (plugin_user_t *user, void *priv, unsigned long event, buffer_t *token) {
	...
	*c = '\0';
	...
	if (!t) return PLUGIN_RETVAL_DROP;
...
unsigned long cmd_parser_main (plugin_user_t *user, void *priv, unsigned long event, buffer_t *token) {
