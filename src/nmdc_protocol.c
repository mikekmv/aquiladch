...
int proto_nmdc_user_forcemove (user_t *u, unsigned char *destination, buffer_t *message) {
  ...
  DPRINTF ("Redirecting user %s to %s because %.*s\n", u->nick, destination, (int)bf_used (message), message->s);
...
int proto_nmdc_handle_token (user_t * u, buffer_t * b)
{
  ...
  if (b) {
    t = b->s;
    if (token_parse (&tkn, t) == TOKEN_UNIDENTIFIED) {
      if (u->state == PROTO_STATE_ONLINE)
  ...
  switch (u->state) {
	...
	if ((a = account_find (u->nick))) {
	  if (a->passwd[0]) {
	    ...
	    proto_nmdc_user_say_string (HubSec, output, "Your account priviliges will not be awarded until you set a password.");
	  }
	}

	if ((!cloning) && (existing_user = hash_find_ip (&hashlist, u->ipaddress))) {
	  proto_nmdc_user_redirect (u, bf_buffer("Cloning is not allowed. Another user is already logged in from this IP address."));
	  retval = -1;
	  break;
	}
	...
	if (ban) {
	  DPRINTF ("** Refused user %s. IP Banned because %.*s\n", u->nick, (unsigned int)bf_used (ban->message), ban->message->s);
	  bf_strcat (output, "<");
	...
	if (nban) {
	  DPRINTF ("** Refused user %s. Nick Banned because %.*s\n", u->nick, (unsigned int)bf_used (nban->message), nban->message->s);
	  bf_strcat (output, "<");
	  bf_strcat (output, HubSec->nick);
	  bf_strcat (output, "> You have been banned because: ");
	...
	/* DPRINTF (" - User %s greeted.\n", u->nick); */
      ...
      break;
    case PROTO_STATE_WAITPASS:	/* waiting for user $ValidateNick */
	...
	if (!a->passwd[0])
	  proto_nmdc_user_say (HubSec, output, bf_buffer("Your account priviliges will not be awarded until you set a password."));
	...
	case TOKEN_MYINFO:
	  {
	    ...
	    if (!u->MyINFO || (u->active < 0)) {
	      if (u->active < 0) {
	     	if (u->rights & CAP_TAG) {
	     		...
	     		proto_nmdc_user_say (HubSec, output, bf_buffer("WARNING: You should use a client that uses tags!"));
	    		u->active = 0;
	    	} else {
	    		proto_nmdc_user_redirect (u, bf_buffer("This hub requires tags, please upgrade to a client that supports them."));
	    		retval = -1;
	    		nmdc_stats.notags++;
			break;
		}
	    ...
	    if (plugin_send_event (u->plugin_priv, PLUGIN_EVENT_PRELOGIN, u->MyINFO) != PLUGIN_RETVAL_CONTINUE) {
		proto_nmdc_user_redirect (u, bf_buffer("Your login was refused."));
		retval = -1;
	    ...
	    DPRINTF (" - User %s has %s shared and is %s\n", u->nick, format_size(u->share), u->active ? "active" : "passive");
	...
	case TOKEN_CHAT:
	{
	  ...
	  /* verify the nick */
	  if (strncasecmp (u->nick, t+1, strlen (u->nick))) {
	...
	case TOKEN_MYINFO:
	{
	  ...
	  if (plugin_send_event (u->plugin_priv, PLUGIN_EVENT_INFOUPDATE, b) != PLUGIN_RETVAL_CONTINUE) {
	  	...
	  	proto_nmdc_user_redirect (u, bf_buffer("Sorry, you no longer satisfy the necessary requirements for this hub."));
	  	break;
	  }
	...
	case TOKEN_GETINFO:
	  {
	    ...
	    /* check quota */
	    if ((u->supports & NMDC_SUPPORTS_NoGetINFO)&&(!get_token (&rates.getinfo, &u->rate_getinfo, now.tv_sec)))
	    	break;
	...
	case TOKEN_KICK:
	{
	  ...
	  bf_printf (output, "You were kicked.");
          banlist_add (&softbanlist, target->ipaddress, output, now.tv_sec + config.defaultKickPeriod);
          banlist_nick_add (&nickbanlist, target->nick, output, now.tv_sec + config.defaultKickPeriod);
	...
	case TOKEN_BOTINFO:
	{
	  ...
	  bf_printf (output, "$HubINFO %s$%s:%d$%s$%lu$%llu$%lu$%lu$%s$%s|", 
	  	...
	  	HUBSOFT_NAME,
	  	owner ? *owner->val.v_string : (unsigned char *) "Unknown"
	  	);
...
unsigned int proto_nmdc_build_buffer (unsigned char *buffer, user_t * u, unsigned int as, unsigned int ps, unsigned int ch, unsigned int pm, unsigned int res)
{
  buffer_t *b;
  unsigned long l;
  unsigned char *t;
...
void proto_nmdc_flush_cache ()
{
  ...
  buf_exception = bf_alloc (((asl > psl) ? asl : psl) + cache.results.length+cache.results.messages.count+cache.privatemessages.length+cache.privatemessages.messages.count);
  ...
  if (cache.privatemessages.messages.count && get_token (&cache.privatemessages.timertype, &cache.privatemessages.timer, now.tv_sec))
  	pm=1;
  
  /* check to see if we need to send results */
  if (cache.results.messages.count && get_token (&cache.results.timertype, &cache.results.timer, now.tv_sec))
  	res=1;
  ...
  if (cache.ZlineSupporters > 0) {
   ...
   if (q)	nmdc_stats.cache_quit 		+= cache.quit.length   		+ cache.quit.messages.count;
   if (mi)	nmdc_stats.cache_myinfo 	+= cache.myinfo.length 		+ cache.myinfo.messages.count;
   if (miu)	nmdc_stats.cache_myinfoupdate	+= cache.myinfoupdate.length 	+ cache.myinfoupdate.messages.count;
   if (ch)	nmdc_stats.cache_chat		+= cache.chat.length    	+ cache.chat.messages.count;
   if (as)	nmdc_stats.cache_asearch	+= cache.asearch.length 	+ cache.asearch.messages.count;
   if (ps)	nmdc_stats.cache_psearch	+= cache.psearch.length 	+ cache.psearch.messages.count;
   if (pm)	nmdc_stats.cache_messages	+= cache.privatemessages.length + cache.privatemessages.messages.count;
   if (res)	nmdc_stats.cache_results	+= cache.results.length 	+ cache.results.messages.count;
...
#ifdef ZLINES
  if (buf_zpassive && (buf_zpassive != buf_passive)) bf_free (buf_zpassive);
  if (buf_zactive  && (buf_zactive  != buf_active))  bf_free (buf_zactive);
#endif
  ...
  if (ch)   cache_clear (cache.chat);
  if (mi)   cache_clear (cache.myinfo);
  if (miu)  cache_clear (cache.myinfoupdate);
  if (miuo) cache_clear (cache.myinfoupdateop);
  if (ps)   cache_clear (cache.psearch);
  if (as)   cache_clear (cache.asearch);
  if (res)  cache_clear (cache.results);
  if (pm)   cache_clear (cache.privatemessages);
  if (q)    cache_clear (cache.quit);
...
int proto_nmdc_init ()
{
  ...
  config_register ("rate.chat.period",		CFG_ELEM_ULONG,  &rates.chat.period, 		  "Period of chat messages. This controls how often a user can send a chat message. Keep this low.");
  config_register ("rate.chat.burst",		CFG_ELEM_ULONG,  &rates.chat.burst, 		  "Burst of chat messages. This controls how many chat messages a user can 'save up'. Keep this low.");
  config_register ("rate.search.period",	CFG_ELEM_ULONG,  &rates.search.period, 		  "Period of searches. This controls how often a user can search. Keep this reasonable.");
  config_register ("rate.search.burst",		CFG_ELEM_ULONG,  &rates.search.burst, 		  "Burst of searches. This controls how many searches a user can 'save up'. Keep this low.");
  config_register ("rate.myinfo.period",	CFG_ELEM_ULONG,  &rates.myinfo.period, 		  "Period of MyINFO messages. This controls how often a user can send a MyINFO message. Keep this very high.");
  config_register ("rate.myinfo.burst",		CFG_ELEM_ULONG,  &rates.myinfo.burst, 		  "Burst of MyINFO messages. This controls how many MyINFO messages a user can 'save up'. Keep this at 1.");
  config_register ("rate.getnicklist.period",	CFG_ELEM_ULONG,  &rates.getnicklist.period, 	  "Period of nicklist requests. This controls how often a user can refresh his userlist. Keep this high.");
  config_register ("rate.getnicklist.burst",	CFG_ELEM_ULONG,  &rates.getnicklist.burst, 	  "Burst of nicklist requests. This controls how many userlist refreshes a user can 'save up'. Keep this at 1.");
  config_register ("rate.getinfo.period",	CFG_ELEM_ULONG,  &rates.getinfo.period, 	  "Period of getinfo requests. This controls how often a user can request info on a user. Keep this low.");
  config_register ("rate.getinfo.burst",	CFG_ELEM_ULONG,  &rates.getinfo.burst, 		  "Burst of getinfo requests. This controls how many getinfo requests a user can 'save up'.");
  config_register ("rate.download.period",	CFG_ELEM_ULONG,  &rates.downloads.period, 	  "Period of downloads. This controls how often a user can initiate a download. Keep this low.");
  config_register ("rate.download.burst",	CFG_ELEM_ULONG,  &rates.downloads.burst, 	  "Burst of downloads. This controls how many downloads a user can 'save up'. Keep this reasonable.");
  config_register ("rate.connect.period",	CFG_ELEM_ULONG,  &rates.connects.period, 	  "Period of connects. This controls how often the connect counter is refreshed. Keep this low.");
  config_register ("rate.connect.burst",	CFG_ELEM_ULONG,  &rates.connects.burst, 	  "Burst of connects. This controls how many new user connects can be saved up in idle time. Keep this low.");
  config_register ("rate.connect.refill",	CFG_ELEM_ULONG,  &rates.connects.refill, 	  "Refill of connects. This controls how many new user connects are added each time the counter resets. Keep this low.");
  config_register ("rate.results.period",	CFG_ELEM_ULONG,  &rates.psresults.period, 	  "Period of passive search results. This controls how often the passive search results counter is refreshed. Keep this low.");
  config_register ("rate.results.burst",	CFG_ELEM_ULONG,  &rates.psresults.burst, 	  "Burst of passive search results. This controls how many passive search results can be saved up in idle time. Keep this low.");
  config_register ("rate.results.refill",	CFG_ELEM_ULONG,  &rates.psresults.refill, 	  "Refill of passive search results. This controls how many passive search results are added each time the counter resets. Keep this low.");
  ...
  config_register ("cache.chat.period",		CFG_ELEM_ULONG,  &cache.chat.timertype.period, 		  "Period of chat cache flush. This controls how often chat messages are sent to users. Keep this low.");
  config_register ("cache.join.period",		CFG_ELEM_ULONG,  &cache.myinfo.timertype.period,	  "Period of join cache flush. This controls how often users are notified of new joins.");
  config_register ("cache.update.period",	CFG_ELEM_ULONG,  &cache.myinfoupdate.timertype.period, 	  "Period of update cache flush. This controls how often users are sent MyINFO updates. Keep this high.");
  config_register ("cache.updateop.period",	CFG_ELEM_ULONG,  &cache.myinfoupdateop.timertype.period,  "Period of operator update cache flush. This controls how often operators are sent MyINFO updates. Keep this low.");
  config_register ("cache.activesearch.period",	CFG_ELEM_ULONG,  &cache.asearch.timertype.period, 	  "Period of active search cache flush. This controls how often search messages are sent to active users.");
  config_register ("cache.passivesearch.period",CFG_ELEM_ULONG,  &cache.psearch.timertype.period, 	  "Period of passive search cache flush. This controls how often search messages are sent to passive users.");
  config_register ("cache.results.period",	CFG_ELEM_ULONG,  &cache.results.timertype.period, 	  "Period of search results cache flush. This controls how often search results are sent to passive users.");
  config_register ("cache.pm.period",		CFG_ELEM_ULONG,  &cache.privatemessages.timertype.period, "Period of private messages cache flush. This controls how often private messages are sent to users. Keep this low.");
  config_register ("cache.quit.period",		CFG_ELEM_ULONG,  &cache.quit.timertype.period,		  "Period of quit cache flush. This controls how often quit messages are sent to users.");
  ...
  config_register ("hub.allowcloning",		CFG_ELEM_UINT,	 &cloning,				  "Allow multiple users from the same IP address.");
  config_register ("nmdc.maxchatlength",	CFG_ELEM_UINT,	 &chatmaxlength,			  "Maximum length of a chat message.");
  config_register ("nmdc.maxsearchlength",	CFG_ELEM_UINT,	 &searchmaxlength,			  "Maximum length of a search message.");
  config_register ("nmdc.maxsrlength",		CFG_ELEM_UINT,	 &srmaxlength,				  "Maximum length of a search result");
