...
typedef struct chatroom {
 ...
 unsigned long pi_chatroom_event_pm (plugin_user_t *user, void *dummy, unsigned long event, buffer_t *token);
...
chatroom_t *chatroom_new (unsigned char *name, unsigned long rights, unsigned long flags, unsigned char *description, plugin_event_handler_t *handler) {
	chatroom_t *room;
...
chatroom_t *chatroom_find (unsigned char *name) {
	...
	for (room = chatrooms.next ; (room != &chatrooms); room = room->next)
		if (!strcasecmp (name, room->name)) return room;
	...
	while (!feof(fp)) {
		...
		if ((room = chatroom_find(name))) {
			if ( (room->rights != rights) ||
			     (room->flags  != flags)  ||
			     (strcmp (room->description, buffer+offset)) ) {
				chatroom_del (room);
				chatroom_new (name, rights, flags, buffer+offset, (plugin_event_handler_t *)&pi_chatroom_event_pm);
			}
		} else
			chatroom_new (name, rights, flags, buffer+offset, (plugin_event_handler_t *)&pi_chatroom_event_pm);
...
unsigned long pi_chatroom_handler_roomadd (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	unsigned long roomflags = 0, roomrights = 0, ncap = 0, counter = 0;

        if (argc < 3) {
                bf_printf (output, "Usage: %s <room> <description> [<private>] [<autoreg>] [<autorights>] [<rights <rights>...>]\n"
	...
	if (argc > 3) {
		...
		if ((argc > counter) && (!strcmp (argv[counter], "rights"))) {
			...
			counter++;
			if (argc > counter)
				command_flags_parse ((command_flag_t *)Capabilities, output, argc, argv, counter, &roomrights, &ncap);
			roomrights &= ~ncap;
			bf_printf (output, "Room requires: ");
			command_flags_print ((command_flag_t *)(Capabilities+CAP_PRINT_OFFSET), output, roomrights);
			bf_strcat (output, "\n");
		}
	}	

	if (!chatroom_new (argv[1], roomrights, roomflags, argv[2], (plugin_event_handler_t *)&pi_chatroom_event_pm)) {
...
unsigned long pi_chatroom_handler_roomdel (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	chatroom_t *room;
...
unsigned long pi_chatroom_handler_roomlist (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	unsigned int count;
	chatroom_t *room;
...
unsigned long pi_chatroom_handler_userstat (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	return 0;
}
...
unsigned long pi_chatroom_event_login (plugin_user_t *user, buffer_t *output, void *dummy, unsigned long event, buffer_t *token) {
	chatroom_t *room;
	chatroom_member_t *member;
	...
	for (room = chatrooms.next; room != &chatrooms; room = room->next) {
		...
		if ( (room->flags & CHATROOM_FLAG_AUTOJOIN_RIGHTS) && (! ((user->rights & room->rights) == room->rights) ))
			continue;
...
unsigned long pi_chatroom_event_logout (plugin_user_t *user, buffer_t *output, void *dummy, unsigned long event, buffer_t *token) {
	chatroom_t *room;
...
unsigned long pi_chatroom_event_save (plugin_user_t *user, buffer_t *output, void *dummy, unsigned long event, buffer_t *token) {
        chatroom_save (SaveFile);
	return PLUGIN_RETVAL_CONTINUE;
}

unsigned long pi_chatroom_event_load (plugin_user_t *user, buffer_t *output, void *dummy, unsigned long event, buffer_t *token) {
...
int pi_chatroom_init () {
	...
	plugin_request (plugin_chatroom, PLUGIN_EVENT_LOGIN,  (plugin_event_handler_t *)&pi_chatroom_event_login);
	plugin_request (plugin_chatroom, PLUGIN_EVENT_LOGOUT, (plugin_event_handler_t *)&pi_chatroom_event_logout);
	plugin_request (plugin_chatroom, PLUGIN_EVENT_SAVE, (plugin_event_handler_t *)&pi_chatroom_event_save);
	plugin_request (plugin_chatroom, PLUGIN_EVENT_LOAD, (plugin_event_handler_t *)&pi_chatroom_event_load);
	...
	command_register ("chatroomlist", &pi_chatroom_handler_roomlist, CAP_CONFIG, "Lists available chatrooms.");
