...
unsigned long pi_user_handler_userstat (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	bf_printf (output, "Unregistered users: %lu / %lu\n"
			...
			users_total, user_unregistered_max + user_registered_max,
			users_peak, users_dropped
		);
...
unsigned long pi_user_event_prelogin (plugin_user_t *user, void *dummy, unsigned long event, buffer_t *token) {
	...
	plugin_user_printf (user, "Sorry, the hub is full. It cannot accept more users from your class.\n");
...
unsigned long pi_user_event_infoupdate (plugin_user_t *user, void *dummy, unsigned long event, buffer_t *token) {
...
unsigned long pi_user_event_logout (plugin_user_t *user, void *dummy, unsigned long event, buffer_t *token) {
	/* if this is a registered user, check for registered/op max */
	if (user->flags & PROTO_FLAG_REGISTERED) {
...
unsigned long pi_user_handler_clientban (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	buffer_t *buf;
	double min, max;
	
	if (argc < 4) {
		bf_printf (output, "Usage: !%s <clienttag> <minversion> <maxversion> <reason>\nUse 0 for version if unimportant.", argv[0]);
		return 0;
	}
	...
	bf_printf (output, "Client \"%s\" (%lf, %lf) added to banned list because: %s\n", argv[1], min, max, argv[4]);
	
	return 0;
}

unsigned long pi_user_handler_clientlist (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	unsigned int i;
	banlist_client_entry_t *b;
	
	dlhashlist_foreach (&clientbanlist, i) {
		dllist_foreach (dllist_bucket(&clientbanlist, i), b) {
			bf_printf (output, "Client \"%s\" (%lf, %lf) because: %.*s\n", b->client, b->minVersion, b->maxVersion, bf_used (b->message), b->message->s);
		}
	}
...
unsigned long pi_user_handler_clientunban (plugin_user_t *user, buffer_t *output, void *dummy, unsigned int argc, unsigned char **argv) {
	if (argc < 2) {
...
unsigned long pi_user_event_save (plugin_user_t *user, void *dummy, unsigned long event, buffer_t *token) {
	banlist_client_save (&clientbanlist, ClientBanFileName);
	return PLUGIN_RETVAL_CONTINUE;
}

unsigned long pi_user_event_load (plugin_user_t *user, void *dummy, unsigned long event, buffer_t *token) {
	banlist_client_clear (&clientbanlist);
	banlist_client_load (&clientbanlist, ClientBanFileName);
	return PLUGIN_RETVAL_CONTINUE;
}

unsigned long pi_user_event_config (plugin_user_t *user, void *dummy, unsigned long event, config_element_t *elem) {
	buffer_t *buf;
	struct rlimit limit;
 	...
 	if ((elem->name[0] != 'u') || strncasecmp (elem->name, "userlimit", 9))
        ...
        if (limit.rlim_cur < (user_unregistered_max+user_registered_max+user_op_max)) {
	 	buf = bf_alloc (1024);
	        ...
	        bf_printf (buf, "WARNING: resourcelimit for this process allows a absolute maximum of %lu users, currently %lu are configured.\n",
